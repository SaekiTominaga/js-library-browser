...
test('no attribute', () => {
	...
	}).toThrow(new Error('The `data-preview` attribute is not set'));
});
...
test('no template', () => {
	...
	}).toThrow(new Error('Element `#template` must be a `<template>` element'));
});
...
describe('template content', () => {
	test('text node', () => {
		...
		expect(() => {
			new Preview('template');
		}).toThrow(new Error('There must be only Element node, comment node, or empty text node within the `<template>` element'));
	});
	...
	test('empty', () => {
		...
		expect(() => {
			new Preview('template');
		}).toThrow(new Error('There must be one `<output>` element within the `<template>` element'));
	});
