...
describe('validate', () => {
	describe('maxFetchCount', () => {
		test('not integer', async () => {
			await expect(closestHTMLPage('http://example.com', { maxFetchCount: 1.5 })).rejects.toThrow(new Error('Argument `maxFetchCount` must be an integer'));
		});

		test('minus', async () => {
			await expect(closestHTMLPage('http://example.com', { maxFetchCount: -1 })).rejects.toThrow(
				new Error('Argument `maxFetchCount` must be greater than or equal to 0'),
			);
		});
	});
});
