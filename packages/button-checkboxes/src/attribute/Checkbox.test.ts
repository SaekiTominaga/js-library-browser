...
test('no attribute', () => {
	expect(() => {
		new Checkbox({});
	}).toThrow(new Error('The `data-control` or `data-controls-class` or `data-controls-name` attribute is not set'));
});
...
describe('id', () => {
	...
	test('no checkbox', () => {
		...
		}).toThrow(new Error('Checkbox does not exist in descendants of the element `#checkboxes`'));
	});
...
describe('class', () => {
	...
	test('not input', () => {
		...
		}).toThrow(new Error('Element `.checkbox` is not a `HTMLInputElement`'));
	});
